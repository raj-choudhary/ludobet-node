...
function project(p, size, fov = 3.5) {
  const scale = (size * 0.46) / (fov - p[2]);
  return [
    size / 2 + p[0] * scale,
    size / 2 - p[1] * scale
  ];
...
function render3DDiceSVG(rotX, rotY, rotZ, size = 120, liftY = 0) {
  // Transform vertices with lift
  const transVerts = VERTICES.map(v => {
    let p = rotateX(v, rotX);
    p = rotateY(p, rotY);
    p = rotateZ(p, rotZ);
    return [p[0], p[1] + liftY, p[2]];
  });
  ...
  const faceList = FACES.map(f => {
    ...
    const dotL = Math.max(0.15, norm[0] * LIGHT_DIR[0] + norm[1] * LIGHT_DIR[1] + norm[2] * LIGHT_DIR[2]);
  ...
  // Realistic contact shadow below the dice
  const shadowScale = Math.max(0.4, 1 - liftY * 0.4);
  const shadowOpacity = Math.max(0.1, 0.45 - liftY * 0.3);
  svgFaces += `<ellipse cx="${size/2}" cy="${size * 0.88}" rx="${size * 0.38 * shadowScale}" ry="${size * 0.11 * shadowScale}" fill="rgba(15,23,42,${shadowOpacity.toFixed(2)})"/>`;

  faceList.forEach(item => {
    const pts = item.face.v.map(idx => `${projVerts[idx][0].toFixed(1)},${projVerts[idx][1].toFixed(1)}`).join(' ');
    
    // Smooth glossy shaded porcelain color
    const brightness = Math.floor(185 + item.dotL * 70);
    const fillHex = `rgb(${brightness}, ${brightness + 2}, ${brightness + 6})`;
    const strokeHex = `rgb(${Math.floor(brightness * 0.72)}, ${Math.floor(brightness * 0.72)}, ${Math.floor(brightness * 0.76)})`;
    ...
    pips.forEach(([pu, pv]) => {
      ...
      const pipRadius = Math.max(3.2, (size * 0.046) / (3.5 - pipPos[2]));
...
async function createGIFFromSVGFrames(svgFrames, width, height, delay = 45, loop = 0) {
  ...
  for (const svg of svgFrames) {
    ...
    // Quantize RGBA into indexed palette with transparency
...
async function generateAllDiceGIFs() {
  const outDir = path.join(__dirname, 'assets');
  const size = 120;
  const numFrames = 18;

  console.log('Generating 3D Tumbling Animated Dice GIFs (1 to 6)...');
  ...
  for (let targetNum = 1; targetNum <= 6; targetNum++) {
    ...
    for (let f = 0; f < numFrames; f++) {
      ...
      // Height elevation during roll
      const liftY = Math.sin(progress * Math.PI) * 0.45;

      const rotX = (1 - ease) * (Math.PI * 4.2) + targetRot[0] - 0.22;
      const rotY = (1 - ease) * (Math.PI * 6.4) + targetRot[1] + 0.32;
      const rotZ = (1 - ease) * (Math.PI * 3.0) + targetRot[2];

      const svg = render3DDiceSVG(rotX, rotY, rotZ, size, liftY);
      svgFrames.push(svg);
    }

    const gifBuffer = await createGIFFromSVGFrames(svgFrames, size, size, 40, 1);
    fs.writeFileSync(path.join(outDir, `dice_roll_${targetNum}.gif`), gifBuffer);
    console.log(`Generated: assets/dice_roll_${targetNum}.gif`);
  }

  // Also generate a continuous rolling loop GIF
  const loopFrames = [];
  const totalLoopFrames = 16;
  for (let f = 0; f < totalLoopFrames; f++) {
    const t = f / totalLoopFrames;
    const rotX = t * Math.PI * 2 - 0.2;
    const rotY = t * Math.PI * 4 + 0.3;
    const rotZ = t * Math.PI * 2;
    const liftY = Math.sin(t * Math.PI * 2) * 0.35;

    const svg = render3DDiceSVG(rotX, rotY, rotZ, size, liftY);
    loopFrames.push(svg);
  }
  const rollingGif = await createGIFFromSVGFrames(loopFrames, size, size, 45, 0);
  fs.writeFileSync(path.join(outDir, 'dice_rolling.gif'), rollingGif);
  console.log('Generated: assets/dice_rolling.gif (Continuous loop)');
}

generateAllDiceGIFs().catch(console.error);
